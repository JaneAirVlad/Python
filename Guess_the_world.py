import random
word_list = ['ЯБЛОКО', 'КНИГА','ДВЕРЬ','ТРАНКВИЛИЗАТОР','БРОНЕТРАНСПОРТЕР','ЛИКВИДАЦИЯ','НАСЕЛЕНИЕ','КОМПЬЮТЕР','ГОСПИТАЛИЗАЦИЯ','ТЕЛЕФОН','ШОКОЛАД']

def get_word():                        # функция получения случайного слова из списка слов
...
while again.lower() == 'д':            # основной цикл программы
    word = get_word()
    play(word)
